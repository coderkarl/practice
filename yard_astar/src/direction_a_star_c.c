...
int rev_order(const void *a, const void *b)
{
	return 1;
}
...
int main()
{
	...
	Path myPath[MAX_PATH_NODES] = {{0,0,0}};
	int nSteps = 0;
	//count = 1;
	if(done)
	{
		...
		//printf("x: %.1f, y: %.1f\n",x1,y1);
		fprintf(fpPath,"%.1f %.1f %d %d %d\n",x1,y1,dCount,nSteps,0);
		myPath[nSteps].pathX = x1;
		myPath[nSteps].pathY = y1;
		myPath[nSteps].pathAction = 0;
		...
		while( (abs(r1-r_init) > 0 || abs(c1-c_init) > 0) && dCount >= -2)
		{
			...
			dCount = dCount - 1;
			nSteps++;
			fprintf(fpPath,"%.1f %.1f  %d  %d  %d\n",x1,y1,dCount,nSteps,-rev_motion);
			myPath[nSteps].pathX = x1;
			myPath[nSteps].pathY = y1;
			myPath[nSteps].pathAction = -rev_motion;
		}
		//plot(x,y,'g')
	}
	//Close files
	...
	//Reverse pathActions, pathX, and pathY	
	qsort(myPath,nSteps+1,sizeof(Path),rev_order);
	/*
	k = nSteps;   // j will Point to last Element
	j = 0;       // i will be pointing to first element
	...
	*/
	...
	{
		printf("%.0f  %.0f  %d\n",myPath[k].pathX,myPath[k].pathY,myPath[k].pathAction);
	}
	return 0;
}
